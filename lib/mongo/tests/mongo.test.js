...
describe('mongo tests', () => {
  test('constructor error should not have valid client', (done) => {
    ...
    try { new Mongo('uri://', {}) } catch (e) { expect(e).toEqual(new Error('MONGO_PARAMS_INVALID')) } //eslint-disable-line
    try { new Mongo(uri, { ttl: 120 }) } catch (e) { expect(e).toEqual(new Error('MONGO_INIT_INDEX')) } //eslint-disable-line
    done()
  })
  ...
  test('lock should lock, unlock should unlock', async () => {
    ...
    mongo.db.collection('locks').insertOne.mockResolvedValue({ insertedId: 'test' })
    await expect(mongo.lock('test')).resolves.toEqual(true)
    // simulate something locked it before
    mongo.db.collection('locks').insertOne.mockRejectedValue(new Error('E11000 duplicate key error'))
    await expect(mongo.lock('test')).resolves.toEqual(false)
    await mongo.unlock('test')
    // simulate a successful lock
    mongo.db.collection('locks').insertOne.mockResolvedValue({ insertedId: 'test' })
    await expect(mongo.lock('test')).resolves.toEqual(true)
    // simulate empty key should error
    await expect(mongo.lock('')).rejects.toEqual(new Error('MONGO_LOCK_KEY_INVALID'))
    await expect(mongo.lock(null)).rejects.toEqual(new Error('MONGO_LOCK_KEY_INVALID'))
    await expect(mongo.lock()).rejects.toEqual(new Error('MONGO_LOCK_KEY_INVALID'))
    await expect(mongo.unlock('')).rejects.toEqual(new Error('MONGO_UNLOCK_KEY_INVALID'))
    await expect(mongo.unlock(null)).rejects.toEqual(new Error('MONGO_UNLOCK_KEY_INVALID'))
    await expect(mongo.unlock()).rejects.toEqual(new Error('MONGO_UNLOCK_KEY_INVALID'))
    // simulate mongo error or anomaly should throw
    mongo.db.collection('locks').insertOne.mockResolvedValue({ insertedId: 'test1' })
    await expect(mongo.lock('test')).rejects.toEqual(new Error('MONGO_LOCK_ANOMALY'))
    mongo.db.collection('locks').insertOne.mockRejectedValue(new Error('error'))
    await expect(mongo.lock('test')).rejects.toEqual(new Error('error'))
  })
})
