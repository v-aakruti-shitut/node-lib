...
describe('mongo tests', () => {
  ...
  test('unknown collection should throw', (done) => {
    ...
    expect(mongo.findOneAndUpdate('unknown', { a: 1 }, { b: 2 })).rejects.toEqual(new Error('MONGO_FINDONEANDUPDATE_PARAMS'))
    done()
  })
  ...
  test('findOneAndUpdate function should return object', (done) => {
    mongo.client.db('db').collection('collection').findOneAndUpdate.mockResolvedValue({})
    expect(mongo.findOneAndUpdate('collection', { a: 1 }, { b: 2 })).resolves.toEqual({})
    expect(mongo.findOneAndUpdate('collection', { a: 1 }, { b: 2 }, { many: true })).resolves.toEqual({})
    done()
  })
  ...
  test('bulkWrite function should throw error when operations is not an array', async () => {
    expect(mongo.bulkWrite('collection', { insertOne: { a: 1 } })).rejects.toEqual(new Error('MONGO_BULKWRITE_PARAMS'))
  })
  test('bulkWrite function should return object', (done) => {
    expect(mongo.bulkWrite('collection', [{ updateOne: { filter: {}, update: { a: 1 } } }])).resolves.toEqual({})
    done()
  })
