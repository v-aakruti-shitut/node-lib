...
class Mongo extends EventEmitter {
    ...
    client.connect().then(() => {
      ...
      client.db(db).command({ ping: 1 }).then(r => {
        ...
        if (options.lockEnable) {
          this.lockCollection = options.lockCollection || 'locks'
          this.db.createIndex(this.lockCollection, { lockTime: 1 }, {
  ...
  database (db) {
    // be careful here. ideally, if an application need to access multiple databases, better
    // to initialize multiple instances then shifting databases on the fly
    // for example: this can have impact to initialization routines like locks
  ...
  async findOneAndUpdate (collection, query, operation, options = {}) {
    const col = this.db.collection(collection)
    if (!col || !query || !operation) throw new Error('MONGO_FINDONEANDUPDATE_PARAMS')
    return mongoQuery(col, 'findOneAndUpdate', query, operation, options)
  }
  ...
  async bulkWrite (collection, query, options = {}) {
    const col = this.db.collection(collection)
    // query should be an array
    if (!col || !Array.isArray(query)) throw new Error('MONGO_BULKWRITE_PARAMS')
    return mongoQuery(col, 'bulkWrite', query, null, options)
  }

  async lock (key, options = {}) {
    if (!this.lockCollection) throw new Error('MONGO_LOCK_DISABLED')
    ...
    const { data, error } = await common.awaitWrap(this.insert(this.lockCollection, {
      _id: key,
      value: options.value,
      lockTime: new Date()
    }), { timeout: 1000 })
  ...
  async unlock (key, options = {}) {
    ...
    return this.delete(this.lockCollection, { _id: key }, options)
  }
}
