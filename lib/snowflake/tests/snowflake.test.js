...
const VALID_URI = 'snowflake://test_username:test_password@test_account.snowflakecomputing.com/?role=test_role&warehouse=test_warehouse'
...
describe('constructor()', () => {
  ...
  test('should create snowflake connection with database arg if provided in uri', (done) => {
    const uriWithDb = 'snowflake://test_username:test_password@test_account.snowflakecomputing.com/?role=test_role&warehouse=test_warehouse&database=test_database'
  ...
  test('should create snowflake connection with schema arg if provided in uri', (done) => {
    const uriWithDb = 'snowflake://test_username:test_password@test_account.snowflakecomputing.com/?role=test_role&warehouse=test_warehouse&database=test_database&schema=test_schema'
    const snow = new Snowflake(uriWithDb) //eslint-disable-line
    expect(snowflakeSdk.createConnection).toBeCalledWith({
      account: 'test_account',
      password: 'test_password',
  ...
  test('should throw on invalid uri', (done) => {
    ...
    try { new Snowflake(uriNoAccount) } catch (e) { expect(e).toEqual(new Error()) } //eslint-disable-line
    done()
  })
  test('should throw missing account error on incorrect host', (done) => {
    const uriNoAccount = 'snowflake://test_username:test_password@test_account.snow'
    try { new Snowflake(uriNoAccount) } catch (e) { expect(e).toEqual(new Error('SNOWFLAKE_ACCOUNT_EMPTY')) } //eslint-disable-line
    done()
  })
  test('should throw missing username error on missing username', (done) => {
    const uriNoUsername = 'snowflake://:test_password@test_account.snowflakecomputing.com'
    try { new Snowflake(uriNoUsername) } catch (e) { expect(e).toEqual(new Error('SNOWFLAKE_USERNAME_EMPTY')) } //eslint-disable-line
    done()
  })
  test('should throw missing password error on missing password', (done) => {
    const uriNoPassword = 'snowflake://test_username@test_account.snowflakecomputing.com'
    try { new Snowflake(uriNoPassword) } catch (e) { expect(e).toEqual(new Error('SNOWFLAKE_PASSWORD_EMPTY')) } //eslint-disable-line
    done()
  })
  test('should throw missing role error on missing role', (done) => {
    const uriNoRole = 'snowflake://test_username:test_password@test_account.snowflakecomputing.com'
    try { new Snowflake(uriNoRole) } catch (e) { expect(e).toEqual(new Error('SNOWFLAKE_ROLE_EMPTY')) } //eslint-disable-line
    done()
  })
  test('should throw missing warehouse error on missing warehouse', (done) => {
    const uriNoWarehouse = 'snowflake://test_username:test_password@test_account.snowflakecomputing.com/?role=test_role'
    try { new Snowflake(uriNoWarehouse) } catch (e) { expect(e).toEqual(new Error('SNOWFLAKE_WAREHOUSE_EMPTY')) } //eslint-disable-line
    done()
  })
  test('should log init error if incorrect account provided', (done) => {
    const uriIncorrectAccount = 'snowflake://test_username:test_password@incorrect.snowflakecomputing.com/?role=test_role&warehouse=test_warehouse'
    const log = new Log.Standard({ json: false })
    const snow = new Snowflake(uriIncorrectAccount, { log })
    setTimeout(() => {
      expect(snow.log).toEqual(log)
      expect(console.error).toHaveBeenCalledWith(expect.anything(), 'SNOWFLAKE_INIT_CONNECT', 'error')
      done()
    }, 1000)
  })
})
...
describe('parseConnStr()', () => {
  ...
  test('should return null account on invalid host in connection string', (done) => {
    const username = 'test_username'
    const password = 'test_password'
    const account = 'test_account'
    const role = 'test_role'
