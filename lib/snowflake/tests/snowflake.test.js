...
describe('parseConnStr()', () => {
  test('happy path should destroy conn successfully', async (done) => {
    const sf = new Snowflake(VALID_URI, { log })
    const isTermSuccess = await sf.cleanup()
    expect(isTermSuccess).toEqual(true)
    done()
  })
  test('should not destroy conn when conn is not up', async (done) => {
    snowflakeSdk.createConnection.mockImplementation((options) => {
      const { account, username, password } = options
      return {
        connect: (x) => {
  ...
  test('should not destroy conn on error', async (done) => {
    snowflakeSdk.createConnection.mockImplementation((options) => {
      const { account, username, password } = options
      return {
        connect: (x) => {
    ...
    const sf = new Snowflake(VALID_URI, { log })
    const isTermSuccess = await sf.cleanup()
    expect(isTermSuccess).toEqual(false)
    done()
  })
})
