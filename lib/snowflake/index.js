...
class Snowflake extends EventEmitter {
  ...
  /**
   * Cleans up snowflake connection
   * @param {*} cb callback function - 1st arg: error, 2nd arg: boolean on whether connection is successfully terminated
   */
  cleanup (cb) {
    if (this.conn.isUp()) {
      this.conn.destroy((err, conn) => {
        if (err) {
          this.log.error(
            ...
            `Unable to disconnect from Snowflake: ${err.message}`
          )
          cb(err, false)
        } else {
          this.log.out(
            'snowflake disconnect',
            `Disconnected from Snowflake with conn id: ${conn.getId()}`
          )
          cb(null, true)
        }
      })
