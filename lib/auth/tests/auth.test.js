...
describe('auth tests', () => {
  ...
  test('new instance with parameters should have it defined', (done) => {
    ...
    const ggle = new Auth.Google('tenant', 'service', 'domain', { log: new Log.Standard() })
  ...
  it('generateToken should return token object, google return error', async (done) => {
    try { await google.generateToken('id', 'secret') } catch (e) { expect(e).toEqual(new Error('[AUTH] GENERATE_TOKEN_NOT_IMPLEMENTED')) }
  ...
  it('invalid functions should return error', async (done) => {
    try { await google.refreshToken('token') } catch (e) { expect(e).toEqual(new Error('[AUTH] REFRESH_TOKEN_NOT_IMPLEMENTED')) }
    try { await freeipa.refreshToken('token') } catch (e) { expect(e).toEqual(new Error('[AUTH] REFRESH_TOKEN_NOT_IMPLEMENTED')) }

    try { await google.getScope('token') } catch (e) { expect(e).toEqual(new Error('[AUTH] GET_SCOPE_NOT_IMPLEMENTED')) }
    try { await freeipa.getScope('token') } catch (e) { expect(e).toEqual(new Error('[AUTH] GET_SCOPE_NOT_IMPLEMENTED')) }
    done()
  })
  it('invalid functions which are overriden should return error', async (done) => {
    const methods = new Methods({})
    try { await methods.isTokenValid('token') } catch (e) { expect(e).toEqual(new Error('[AUTH] TOKEN_VALID_NOT_IMPLEMENTED')) }
    try { await methods.isOperationAllowed('token') } catch (e) { expect(e).toEqual(new Error('[AUTH] IS_ALLOWED_NOT_IMPLEMENTED')) }
    try { await methods.getMetadata('token') } catch (e) { expect(e).toEqual(new Error('[AUTH] GET_METADATA_NOT_IMPLEMENTED')) }
    done()
  })
  it('isTokenValid should return object with at least id and domain', async (done) => {
    ...
    try { await google.isTokenValid('empty') } catch (e) { expect(e).toEqual(new Error('[AUTH_ID] UNKNOWN_USER')) }
    try { await freeipa.isTokenValid('empty') } catch (e) { expect(e).toEqual(new Error('[AUTH_ID] UNKNOWN_USER')) }

    try { await google.isTokenValid('error') } catch (e) { expect(e).toEqual(new Error('[AUTH_ID] [AUTH_INFO] TEST')) }
    try { await freeipa.isTokenValid('error') } catch (e) { expect(e).toEqual(new Error('[AUTH_ID] [AUTH_INFO] TEST')) }

    // this test case is only applicable on google
    try { await google.isTokenValid('domainmatch') } catch (e) { expect(e).toEqual(new Error('[AUTH_ID] INVALID_DOMAIN')) }
    try { await google.isTokenValid('domainwrong') } catch (e) { expect(e).toEqual(new Error('[AUTH_ID] INVALID_DOMAIN')) }
  ...
  it('isOperationAllowed should return boolean or error', async (done) => {
    ...
    try { await freeipa.isOperationAllowed('error') } catch (e) { expect(e).toEqual(new Error('[AUTH_IS_ALLOWED] [AUTH_GROUPS] ERROR')) }
    try { await google.isOperationAllowed('error') } catch (e) { expect(e).toEqual(new Error('[AUTH_IS_ALLOWED] [AUTH_GROUPS] ERROR')) }
    done()
  })
  it('getMetadata should return object or error', async (done) => {
    ...
    try { await freeipa.getMetadata('error') } catch (e) { expect(e).toEqual(new Error('[AUTH_METADATA_ID] [AUTH_INFO] ERROR')) }
    try { await google.getMetadata('error') } catch (e) { expect(e).toEqual(new Error('[AUTH_METADATA_ID] [AUTH_INFO] ERROR')) }
    done()
  })
  it('empty getMetadata http response should be handled accordingly', async (done) => {
    ...
    try { await freeipa.generateToken() } catch (e) { expect(e).toEqual(new Error('[AUTH_LOGIN] ERROR')) }
    try { await freeipa.isTokenValid() } catch (e) { expect(e).toEqual(new Error('[AUTH_ID] [AUTH_INFO] EMPTY')) }

    try { await freeipa.getMetadata('empty', 'id') } catch (e) { expect(e).toEqual(new Error('[AUTH_GROUPS] EMPTY')) }
    try { await google.getMetadata('empty', 'id') } catch (e) { expect(e).toEqual(new Error('[AUTH_GROUPS] EMPTY')) }

    try { await freeipa.getMetadata('unknown') } catch (e) { expect(e).toEqual(new Error('[AUTH_GROUPS] EMPTY')) }
    try { await google.getMetadata('unknown') } catch (e) { expect(e).toEqual(new Error('[AUTH_GROUPS] EMPTY')) }
    done()
  })
