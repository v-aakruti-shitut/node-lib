...
class Redis extends methods {
  ...
  constructor (redisuri, options = {}) {
    ...
    if (options.cluster === false) {
      ...
      // remove if not elasticache
      if (options.dnsLookup === false) delete options.redisOptions.dnsLookup
      if (options.tls === false) delete options.redisOptions.redisOptions.tls
