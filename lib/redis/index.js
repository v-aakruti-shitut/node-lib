...
class Redis extends methods {
  ...
  constructor (redisuri, clientOptions, options = {}) {
    ...
    // check if the constructor was provided clientOptions
    // if not, we assume cluster mode is enabled
    if (!clientOptions) clientOptions = Redis.clientOptions(true)

    const client = new Ioredis(redisuri, clientOptions)

    // optional pubsub which application developer needs to explicitly enable
    const subscriber = options.pubsub ? new Ioredis(redisuri, clientOptions) : undefined
