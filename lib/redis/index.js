...
class Redis extends methods {
  ...
  constructor (redisuri, clientOptions, options = {}) {
    ...
    if (options.cluster === false) {
      ...
      if (!clientOptions) clientOptions = Redis.clientOptions(url.password)
      const nodes = [{ host: url.hostname, port: url.port }]
      client = new Ioredis.Cluster(nodes, clientOptions)
      subscriber = options.pubsub ? new Ioredis.Cluster(nodes, clientOptions) : undefined
    }
